...
function rainSwitch() {
    ...
    var canvas2 = $('#canvas-box canvas')[0];
    var ctx = canvas2.getContext('2d');
    var w = 750 || window.screen.width;
    var h = 1440 || window.screen.height;
    canvas2.width = w;
    canvas2.height = h;
    ...
    function particle() {
        ...
        p.setParticle = function () {
            p.position = {
                x: 40 + (Math.random() * (canvas2.width - 40)),
                y: -10 - (Math.random() * 50),
                z: (Math.random() * 10)
            };
            p.velocity = {
    ...
    function checkFloorCollision(i, nextPy) {
        if (nextPy >= canvas2.height - (particles[i].position.z * (canvas2.height / 15))) {
